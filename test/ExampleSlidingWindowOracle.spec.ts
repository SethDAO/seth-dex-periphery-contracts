...
import { SethPairFactory, SethPair } from "@sethdao/dex-core-contracts";

import { v2Fixture } from "./shared/fixtures";
import { bigNumberify, expandTo18Decimals, mineBlock, encodePrice } from "./shared/utilities";
import { ExampleSlidingWindowOracle__factory, IERC20, WETH9 } from "../types";
...
describe("ExampleSlidingWindowOracle", () => {
    ...
    async function addLiquidity(
        amount0: BigNumber = defaultToken0Amount,
        amount1: BigNumber = defaultToken1Amount
    ) {
    ...
    function deployOracle(windowSize: number, granularity: number) {
        return new ExampleSlidingWindowOracle__factory(wallet).deploy(
            factory.address,
            windowSize,
            granularity,
            overrides
    ...
    it("requires granularity to be greater than 0", async () => {
        await expect(deployOracle(defaultWindowSize, 0)).to.be.revertedWith(
    ...
    describe("#observationIndexOf", () => {
        ...
        it("matches offline computation", async () => {
            const oracle = await deployOracle(defaultWindowSize, defaultGranularity);
            for (const timestamp of [0, 5000, 1000, 25000, 86399, 86400, 86401]) {
                expect(await oracle.observationIndexOf(timestamp)).to.eq(
                    observationIndexOf(timestamp)
                );
            }
        });
    });
    ...
    describe("#update", () => {
        ...
        it("sets the appropriate epoch slot", async () => {
            ...
            await slidingWindowOracle.update(token0.address, token1.address, overrides);
            expect(
                await slidingWindowOracle.pairObservations(
                    pair.address,
                    observationIndexOf(blockTimestamp)
                )
            ).to.deep.eq([
        ...
        it("gas for first update (allocates empty array)", async () => {
            ...
            expect(receipt.gasUsed).to.eq("117287");
        ...
        it("gas for second update in the same period (skips)", async () => {
            ...
            const tx = await slidingWindowOracle.update(token0.address, token1.address, overrides);
            const receipt = await tx.wait();
            expect(receipt.gasUsed).to.eq("25673");
        }).retries(2); // gas test inconsistent
        ...
        it("gas for second update different period (no allocate, no skip)", async () => {
            ...
            const tx = await slidingWindowOracle.update(token0.address, token1.address, overrides);
            const receipt = await tx.wait();
            expect(receipt.gasUsed).to.eq("94958");
        }).retries(2); // gas test inconsistent

        it("second update in one timeslot does not overwrite", async () => {
            await slidingWindowOracle.update(token0.address, token1.address, overrides);
            const before = await slidingWindowOracle.pairObservations(
                pair.address,
                observationIndexOf(0)
            );
            // first hour still
            await mineBlock(provider, startTime + 1800);
            await slidingWindowOracle.update(token0.address, token1.address, overrides);
            const after = await slidingWindowOracle.pairObservations(
                pair.address,
                observationIndexOf(1800)
            );
    ...
    describe("#consult", () => {
        ...
        it("fails if previous bucket not set", async () => {
            await slidingWindowOracle.update(token0.address, token1.address, overrides);
            await expect(
                slidingWindowOracle.consult(token0.address, 0, token1.address)
            ).to.be.revertedWith("SlidingWindowOracle: MISSING_HISTORICAL_OBSERVATION");
        });

        it("fails for invalid pair", async () => {
            await expect(slidingWindowOracle.consult(weth.address, 0, token1.address)).to.be
                .reverted;
        });
        ...
        describe("happy path", () => {
            ...
            it("provides the current ratio in consult token0", async () => {
                expect(
                    await slidingWindowOracle.consult(token0.address, 100, token1.address)
                ).to.eq(200);
            });

            it("provides the current ratio in consult token1", async () => {
                expect(
                    await slidingWindowOracle.consult(token1.address, 100, token0.address)
                ).to.eq(50);
            });
        });
